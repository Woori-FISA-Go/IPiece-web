...
export default function NewContestPage() {
  ...
  const [status, setStatus] = useState<'offering' | 'ended' | 'market'>(
    'offering',
  );
  ...
  const isOfferingQuantityMismatch = useMemo(() => {
    ...
    if (Number.isNaN(tokenQuantity) || Number.isNaN(offeringAmount))
      return false;
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!isTokenCreated) {
      toast({
        ...
        description:
          '프로젝트 토큰 생성 완료 후 공모 상품을 등록할 수 있습니다.',
      });
      return;
    }
    if (isOfferingQuantityMismatch) {
      toast({
        ...
        description:
          '토큰 정보의 발행 수량과 공모 모집 수량이 일치해야 합니다.',
      });
      return;
    }
    ...
    if (
      !formState.presentImg ||
      !formState.thumbnailImg ||
      !formState.detailImg
    ) {
      toast({
        title: '이미지를 업로드해 주세요',
    ...
    try {
      const payload = {
        ...
        issue_date: formState.issueDate
          ? new Date(formState.issueDate).toISOString()
          : null,
      ...
      if (formState.presentImg)
        formData.append('presentImg', formState.presentImg);
      if (formState.thumbnailImg)
        formData.append('thumbnailImg', formState.thumbnailImg);
      if (formState.detailImg)
        formData.append('detailImg', formState.detailImg);
            ...
            <CardContent className="space-y-6 p-6">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="owner"
                    placeholder="예: 소유자 이름 혹은 소유 회사 이름"
                ...
                <div className="md:col-span-2 grid grid-cols-1 gap-6 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      value={
                        offeringTotal !== null
                          ? `${currencyFormatter.format(offeringTotal)} KRW`
                          : ''
                      }
                      placeholder="-"
                      readOnly
                ...
                <div className="space-y-2">
                  ...
                  <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 p-6 text-gray-500 hover:border-[#1A4DE5] hover:text-[#1A4DE5]">
                    ...
                    <Input
                      ...
                      className="hidden"
                      onChange={(event) =>
                        handleImageUpload(event, 'presentImg')
                      }
                    />
                  </label>
                </div>
                <div className="space-y-2">
                  ...
                  <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 p-6 text-gray-500 hover:border-[#1A4DE5] hover:text-[#1A4DE5]">
                    ...
                    <Input
                      ...
                      className="hidden"
                      onChange={(event) =>
                        handleImageUpload(event, 'thumbnailImg')
                      }
                    />
                  </label>
                </div>
                <div className="space-y-2">
                  ...
                  <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-200 bg-gray-50 p-6 text-gray-500 hover:border-[#1A4DE5] hover:text-[#1A4DE5]">
                    ...
                    <Input
                      ...
                      className="hidden"
                      onChange={(event) =>
                        handleImageUpload(event, 'detailImg')
                      }
                    />
                  </label>
                </div>
