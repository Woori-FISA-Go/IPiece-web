...
export default function SignupPage() {
  ...
  const phoneFromVerify = useMemo(
    () => searchParams.get('phone') || '',
    [searchParams],
  );
  const birthFromVerify = useMemo(
    () => searchParams.get('birth') || '',
    [searchParams],
  );
  const verifiedFromVerify = useMemo(
  ...
  const handleCheckUsername = async () => {
    ...
    try {
      ...
      const res = await apiFetch(
        `/v1/signup/duplicate-check?${search.toString()}`,
        {
          method: 'GET',
        },
      );
  ...
  const buildPasswordErrors = (password: string): string[] => {
    ...
    if (!/[A-Z]/.test(password))
      errors.push('비밀번호는 대문자가 1자 이상 포함되어야 합니다.');
    if (!/[a-z]/.test(password))
      errors.push('비밀번호는 소문자가 1자 이상 포함되어야 합니다.');
    if (!/[0-9]/.test(password))
      errors.push('비밀번호는 숫자가 1자 이상 포함되어야 합니다.');
    if (!/[!@#$%^&*(),.?\":{}|<>]/.test(password))
      errors.push('비밀번호는 특수문자가 1자 이상 포함되어야 합니다.');
    return errors;
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const fileName =
        'id_card' + (blob.type.includes('pdf') ? '.pdf' : '.png');
      const idCardFile = new File([blob], fileName, {
      ...
      <div className="w-full max-w-[520px] rounded-[12px] border border-[#E6E9F0] bg-white p-9 shadow-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {usernameAvailable === false && usernameError && (
              <div className="flex items-center gap-2 rounded-md bg-[#FEF1F2] px-3 py-2">
                <img
                  ...
                  height={16}
                />
                <span className={`${helperTextClass} text-[#E83B46]`}>
                  {usernameError}
                </span>
              </div>
            )}
            ...
            {passwordErrors.length > 0 && formData.password && (
              <div className="mt-2 space-y-1">
                {passwordErrors.map((error, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-2 rounded-md bg-[#FEF1F2] px-3 py-2"
                  >
                    <img
                      src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Error-K9PTWKSZz4DbA3eEFw6aS3Om28TIS2.png"
                      alt="Error"
          ...
          <div className="space-y-2">
            ...
            />
            {passwordMatch === false && (
              <p className={`${helperTextClass} text-[#E83B46]`}>
                비밀번호가 일치하지 않습니다.
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <p className="text-sm font-medium text-[#1F2229]">
              ...
              {uploadedFiles.length > 0 ? (
                <p className="mt-3 text-xs text-[#1F2229]">
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent className="w-full max-w-sm gap-0 bg-white p-0 rounded-2xl">
          ...
          <div className="flex flex-col items-center gap-2 px-6 py-8">
            ...
            <Button
              className="h-11 w-full rounded-lg bg-[#3386E5] hover:bg-[#2a75d0] mt-2"
